...
----------------------------------------------------
declare @accion int = 1  -- 0 = Ver / 1 = Borrar
----------------------------------------------------
...
select name 
  ...
  where xtype = 'u' 
	...
	and name not in ('SAACAMPOS','SAAGRUPOS','SAAOPER','SACONF','SACORRELSIS','SAESTA','SAJOIN','SAESTADO','SACIUDAD','SAPAIS','SAMUNICIPIO',
	                 'SAFIEL','SATABL','SAFLDREF','SAITRE','SAREPO','SASUCURSAL')  -- TABLAS BASICAS NO TOCAR
	and name not in ('SATAXES','SAPROD','SATAXPRD','SACODBAR','SACLIE','SAPROV','SATARJ','SAVEND','SAINSTA','SADEPO') -- FILTRO=Agregar las tablas que no quiere truncate para activar eliminar los 2 guiones de esta linea
