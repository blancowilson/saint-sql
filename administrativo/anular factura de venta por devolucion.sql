...
Declare @NumeroR varchar (16),
@NroRegi varchar (10)

SET DATEFORMAT YMD

set @numerod='000179'
set @codclie='J075227719'
set @TipoFac ='B'
set @tipocxc = '31'
...
select @NumeroFact=NumeroR from SAFACT where NumeroD =@NumeroD and TipoFac =@TipoFac
...
--SELECT * FROM SAACXC WHERE NumeroD=@numerod AND TipoCxc='31' AND CodClie=@CodClie
--	--Devolver el saldo a la factura
	...
	FETCH MontosP INTO @NumeroFact,@MontoPagado
		...
		BEGIN	
			update saacxc set Saldo=saldo+@MontoPagado 
			where NumeroD = @NumeroFact and TipoCxc = '10'
	    FETCH MontosP INTO @NumeroFact,@MontoPagado
...
update safact set descrip='ANULADO', monto=0, mtotax=0, tgravable=0, costoprd=0, TExento=0, Fletes=0,
...
update SAFACT 
set NumeroR = NULL
where NumeroD =@NumeroFact and TipoFac ='A'
